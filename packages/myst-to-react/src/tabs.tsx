...
export function TabSet({
  ...
  return (
    <TabSetStateProvider active={active}>
      <div className={classNames('-tab-set my-5', className)}>
        <div className="-tab-set-row flex flex-row flex-wrap overflow-x-auto border-b border-b-gray-100">
          {tabs.map((tab) => {
            return (
              <div
                key={tab.id}
                className={classNames('-tab-item-header flex-none px-3 py-1 font-semibold cursor-pointer', {
                  '-tab-item-header-active text-blue-600 border-b-2 border-b-blue-600 dark:border-b-white dark:text-white':
                    active === tab.id,
                  '-tab-item-header-inactive text-gray-500 dark:text-gray-300 hover:text-gray-700 dark:hover:text-gray-100':
                    active !== tab.id,
                })}
                onClick={() => onClick(tab)}
              >
        ...
        <div className="-tab-item-body flex shadow">
          <div className="-tab-item-text w-full px-6">{children}</div>
        </div>
      </div>
    </TabSetStateProvider>
  );
