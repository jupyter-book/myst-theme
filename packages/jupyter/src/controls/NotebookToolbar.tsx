  ...
  if (computable)
    return (
      <div className="myst-jp-nb-toolbar sticky top-[60px] pb-[14px] flex justify-end w-full z-20 pointer-events-none">
        <div className="myst-jp-nb-toolbar-inner flex p-1 m-1 space-x-1 border rounded-full shadow pointer-events-auto border-myst-border-strong bg-myst-bg/80 backdrop-blur">
          {!ready && (
            <div className="myst-jp-nb-toolbar-power rounded">
              <button
                className={classNames(
                  'myst-jp-nb-toolbar-power-btn flex text-center rounded-full cursor-pointer text-myst-text hover:opacity-100 opacity-60',
                  {
                    'opacity-10 text-myst-text-tertiary': connecting || building,
                  },
                )}
                onClick={handleStart}
                disabled={building}
              ...
              {(connecting || building) && !error && (
                <span
                  className="myst-jp-nb-toolbar-spinner absolute top-1 left-1 z-10 w-[22px] h-[22px] opacity-100"
                  title={title}
                >
                  <Spinner size={24} />
                </span>
